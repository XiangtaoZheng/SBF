...
from mycode.conf import settings
...
def OHS_loader(path: str, bands) -> Any:
    if (bands == 'rgb'):
        bands_range = settings.OHS_RGB_BANDS
    elif (bands == 'tif'):
        bands_range = settings.OHS_TIFF_BANDS
    else:
        bands = bands.split(',')
        bands_range = (int(bands[0]), int(bands[1]), int(bands[2]))
    # print(path)
    ...
    for i in range(len(bands_range)):
        re_im[:, :, i] = toNomr(im[:, :, bands_range[i]], True, 800)
    return re_im.astype('float32')

def hsrs_loader(path: str, bands) -> Any:
    if (bands == 'rgb'):
        bands_range = settings.HSRS_RGB_BANDS
    elif (bands == 'tif'):
        bands_range = settings.HSRS_TIFF_BANDS
    else:
        bands = bands.split(',')
        bands_range = (int(bands[0]), int(bands[1]), int(bands[2]))
    # print(path)
    ...
    re_im = np.zeros([imshape[1], imshape[2], len(bands_range)]).astype('float32')
    for i in range(len(bands_range)):
        re_im[:, :, i] = toNomr(im[bands_range[i], :, :], True, 12000)
    return re_im.astype('float32')
...
def tg1hrssc_vni_loader(path: str, bands) -> Any:
    if (bands == 'rgb'):
        bands_range = settings.TG1HRSSC_VNI_RGB_BANDS
    elif (bands == 'tif'):
        bands_range = settings.TG1HRSSC_VNI_TIFF_BANDS
    else:
        bands = bands.split(',')
        bands_range = (int(bands[0]), int(bands[1]), int(bands[2]))
    # print(path)
...
# 基于dataset中ImageFolder读取特定组织格式的数据集：
# -dataset
# --class1
# ---class1_image_name1.tif
# ---class1_image_name2.tif
# ...
# --class2
# ...
...
# 依据seed对图像进行划分处理，frac为训练集比例
def split_datasets(dataset: tudata.Dataset, frac: float, seed=345235):
    train_size = int(frac * dataset.__len__())
    print(train_size)
    test_size = dataset.__len__() - train_size
    # return tudata.random_split(dataset=dataset,lengths=[train_size, test_size],generator=torch.Generator().manual_seed(seed))
